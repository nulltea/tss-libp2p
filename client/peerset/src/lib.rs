...
use futures::{channel::oneshot, channel::mpsc, prelude::*};
...
impl Peerset {
	...
	pub fn from_config(config: PeersetConfig) -> (Self, PeersetHandle) {
		...
		let mut peerset = {
			...
			Self {
				data: peers_state::PeersState::new(config.sets.iter().map(|set| {
					peers_state::SetConfig { in_peers: set.in_peers, out_peers: set.out_peers }
				})),
				tx,
				rx,
	...
	fn on_remove_reserved_peer(&mut self, set_id: SetId, peer_id: PeerId) {
		...
		// disconnected.
		if let peers_state::Peer::Connected(peer) = self.data.peer(set_id.0, &peer_id) {
			peer.disconnect();
